...
math::vector2 BoxCollider::getSize() const
{
	return math::vector2(_w, _h);
}
...
{
	const math::vector2& circle_position = collider2.getPosition();
	const math::vector2& box_position = getPosition();
	math::vector2 distance = _position - collider2.getPosition();
...
bool BoxCollider::check(const BoxCollider& collider2) const
{
	float w2_1 = _w / 2;
	float h2_1 = _h / 2;
	math::vector2 collider_2_size = collider2.getSize();
	float w2_2 = collider_2_size.x / 2;
	float h2_2 = collider_2_size.y / 2;
	math::vector2 distance = getPosition() - collider2.getPosition();
	return fabs(distance.x) < w2_1 + w2_2 && fabs(distance.y) < h2_1 + h2_2;
}

bool BoxCollider::check(const LevelMapCollider& collider2) const
{
